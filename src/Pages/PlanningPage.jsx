...
export default function Planning(){
    ...
    //instead of saving one emplyee save array:
    ...
    // Function to get selected employees for a specific post and shift
    const getSelectedEmployees = (postId, shiftId, date) => {
    // Make sure the date exists
    if (!planningDataRefs.current || !planningDataRefs.current[date]) return [];

    const key = `${postId}-${shiftId}`;
    const assignments = planningDataRefs.current[date][key];

    // If nothing is assigned, return empty array
    if (!assignments) return [];

    // If assignments is an array of objects with emp_id
    if (Array.isArray(assignments)) {
        return assignments
            .map(a => {
                if (!a || !a.emp_id) return null; // safeguard
                return employees.find(emp => emp.emp_id === a.emp_id) || null;
            })
            .filter(emp => emp !== null);
    }

    // If still a single object (old format), return as array
    if (assignments.emp_id) {
        const emp = employees.find(emp => emp.emp_id === assignments.emp_id);
        return emp ? [emp] : [];
    }
    ...
    try {
        ...
        const planningArray = [];
         ...
         if (planningDataRefs.current?.[currentDate]) {
            // Loop over each key (ex: "1-0", "2-1", etc.)
            Object.entries(planningDataRefs.current[currentDate]).forEach(([key, employees]) => {
                if (!employees) return;

                // Split the key to get postId and shiftId
    ...
    try {
        ...
        for (const date of weekDates) {
            ...
            if (planningDataRefs.current?.[date]) {
                Object.values(planningDataRefs.current[date]).forEach(item => {
                    if (!item) return;
                    ...
                    {posts.map(post => (
                        <tr key={post.id}>
                            ...
                            {shifts.map(shift => (
                                ...
                                {(getSelectedEmployees(post.id, shift.id, getCurrentDate()) || []).map((emp, idx) => (
                                    <div key={idx}>{emp.name}</div>
                                ))}

                                {/* Dropdown to assign new employee */}
                                <DropDownList
                                    employees={employees.filter(e =>
                                        !getSelectedEmployees(post.id, shift.id, getCurrentDate()).some(sel => sel.emp_id === e.emp_id)
                                    )}
                                    onSelect={(employee) => handleEmployeeSelect(post.id, shift.id, employee, getCurrentDate())}
                                    // Remove selectedEmployee prop since we now display them separately
                                />
                                </td>
